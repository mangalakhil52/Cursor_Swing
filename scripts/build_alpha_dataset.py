#!/usr/bin/env python3
"""Build a point-in-time alpha research dataset from persistent OHLCV history."""
...
def main()->int:
    ...
    for path in sorted(root.glob('*.csv')):
        ...
        try:
            ...
            if not frame.empty:
                frame.insert(0,'symbol',path.stem); frames.append(frame); eligible+=1
        except Exception as exc: print(f'skip {path.name}: {exc}')
    if not frames: raise SystemExit('No eligible history files found after cache validation')
    out=pd.concat(frames,ignore_index=True); out_path=Path(args.output); out_path.parent.mkdir(parents=True,exist_ok=True); out.to_parquet(out_path,index=False)
    print(f'Wrote {len(out):,} observations across {out.symbol.nunique():,} symbols from {eligible:,} eligible history files')
    return 0
