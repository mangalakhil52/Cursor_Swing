...
p.add_argument('--input',default='reports/residual_alpha_oos.csv')
p.add_argument('--rank-input',default='reports/cross_sectional_rank.csv')
p.add_argument('--history',default='data/history_daily')
p.add_argument('--output',default='reports/portfolio_candidates.csv')
p.add_argument('--probability-floor',type=float,default=.65)
p.add_argument('--max-disagreement',type=float,default=.12)
p.add_argument('--ensemble-threshold',type=float,default=68.)
...
else:d['regime']='SIDEWAYS'

d['ensemble_score']=[ensemble_score(s,p,r) for s,p,r in zip(d.structural_score,d.execution_probability,d.regime)]
d['eligible']=(d.execution_probability>=a.probability_floor)&(d.model_disagreement<=a.max_disagreement)&(d.ensemble_score>=a.ensemble_threshold)
d['date_rank']=d.groupby('date')['ensemble_score'].rank(method='first',ascending=False)
d['eligible']&=d.date_rank<=a.top_k
