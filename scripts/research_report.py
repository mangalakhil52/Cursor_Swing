...
def main()->None:
    ...
    for p in sorted(Path(args.data_dir).glob('*.csv')):
        ...
        try:
            ...
            if len(d)>=cfg.min_history+cfg.horizon+2:
                b=benchmark.close if benchmark is not None and 'close' in benchmark.columns else None
                r=evaluate_symbol(p.stem.upper(),d,benchmark=b,cfg=cfg,fno_symbols=fno)
    ...
    pd.Series(summarize(result)).to_csv(out.with_name('summary.csv'),header=['value'])
    decile_report(result).to_csv(out.with_name('score_deciles.csv'),index=False)
    symbol_concentration(result).to_csv(out.with_name('symbol_concentration.csv'),index=False)
    block_stability(result).to_csv(out.with_name('time_blocks.csv'),index=False)
    ...
    print(pd.Series(summarize(result)).to_string()); print('\nREGIME x DIRECTION\n'); print(result.groupby(['regime','direction'],observed=True).agg(trades=('r_multiple','size'),win_rate=('r_multiple',lambda s:(s>0).mean()),avg_r=('r_multiple','mean'),cum_r=('r_multiple','sum')).reset_index().to_string(index=False)); print(f'\nF&O short universe: {len(fno)} symbols'); print(f'Saved: {out}')
