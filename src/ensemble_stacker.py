...
def build_stack(df):
    ...
    return x[BASE_COLS].apply(pd.to_numeric,errors='coerce').replace([np.inf,-np.inf],np.nan)
...
def expanding_stack(df:pd.DataFrame,min_train=200,step=25):
    ...
    for i in range(min_train,len(d),step):
        j=min(i+step,len(d)); train=X.iloc[:i].copy(); test=X.iloc[i:j].copy(); train=train.fillna(train.median()).fillna(.5); test=test.fillna(train.median()).fillna(.5)
