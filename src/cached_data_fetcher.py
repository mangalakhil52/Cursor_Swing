"""Persistent incremental daily OHLCV cache for the GitHub Actions scanner."""
...
class CachedDataFetcher(DataFetcher):
    ...
    def __init__(self, exchange_suffix='.NS', cache_dir='data/history_daily', history_days=365, refresh_days=10):
        super().__init__(exchange_suffix)
        self.cache_dir=Path(cache_dir); self.history_days=int(history_days); self.refresh_days=int(refresh_days)

    def _path(self,symbol):
        safe=str(symbol).replace('^','INDEX_').replace('/','_').replace('\\','_')
    ...
    def _read(self,symbol):
        ...
        try:
            ...
            return self._normalize_single(x)
        except Exception:
            return pd.DataFrame()
    ...
    def _write(self,symbol,df):
        ...
        x=df.sort_index(); x=x[~x.index.duplicated(keep='last')]
        x.to_csv(self._path(symbol),date_format='%Y-%m-%dT%H:%M:%S%z')
    ...
    def fetch_daily_batch(self,symbols,days=90,chunk_size=80):
        ...
        for s in symbols:
            ...
            out[s]=old
            if (today-old.index[-1].date()).days>=2: warm.append(s)
        if cold:
        ...
        if warm:
            ...
            for s,df in fresh.items():
                merged=pd.concat([cached[s],df]).sort_index()
                merged=merged[~merged.index.duplicated(keep='last')]
                self._write(s,merged); out[s]=merged
        return {s:d for s,d in out.items() if d is not None and not d.empty}

    def fetch_daily(self,symbol,days=90):
        return self.fetch_daily_batch([symbol],days=days).get(symbol,pd.DataFrame())
