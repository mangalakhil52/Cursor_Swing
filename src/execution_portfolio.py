...
def simulate_candidates(candidates: pd.DataFrame, bars_by_symbol: dict[str,pd.DataFrame], capital: float=1_000_000., max_positions: int=2):
    ...
    for _,c in candidates.sort_values(['entry_date','score'],ascending=[True,False]).iterrows():
        active=[a for a in active if a['exit_date']>=c.entry_date]
        ...
        path=simulate_trade(bars.loc[pd.to_datetime(bars.index)>=pd.to_datetime(c.entry_date)],str(c.direction),float(c.entry_price),float(c.stop_price),float(c.target1),float(c.target2),PathConfig())
        z=c.to_dict(); z.update(s); z.update(path); z['pnl_cash']=float(z['net_r'] if 'net_r' in z else path['r_multiple'])*capital*s['risk_pct']; accepted.append(z)
