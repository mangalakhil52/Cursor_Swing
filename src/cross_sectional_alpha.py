...
def walk_forward_score(df:pd.DataFrame,date_col='date',outcome_col='target_before_stop',min_train_dates=30)->pd.DataFrame:
    ...
    for i,dt in enumerate(dates):
        ...
        if len(train)<100 or y.nunique()<2: continue
        model=HistGradientBoostingClassifier(max_iter=150,learning_rate=.05,max_leaf_nodes=15,l2_regularization=2.,random_state=42)
